...
After the write back concludes, the writer then clears the header field for all objects it has modified, resets 
the writer timestamp to infinity, and then reclaims the private object copies in the log buffer.
