...
must not directly overwrite data from older epoches. In ThyNVM, to write ordering constraints must be observed. The first 
...
the store operation is allowed to commit.