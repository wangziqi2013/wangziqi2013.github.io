...
storing epoches and other data that does not have a version in the two epoch areas. Note that during normal operation,
ThyNVM only maintains NVM image for the last epoch and the penultimate epoch, 