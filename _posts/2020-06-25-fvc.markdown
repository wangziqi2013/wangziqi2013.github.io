...
each cache line in the set, only (n + 1) bits from the first sub-bank is read. Note that the paper assumes that tag
access is slower than accessing the (n + 1) bit sub-bank, probably because the tags are longer than (n + 1) bits. 
