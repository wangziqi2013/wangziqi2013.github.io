...
written to files in order to maintain the abstraction of atomic file write system call. If the database uses a file to
store the persistent log, then the I/O operation to persiste the log will be duplicated due to file system level journaling,
