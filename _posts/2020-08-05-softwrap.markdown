...
To make shadow page checkpointing truly non-blocking and a background process, the paper proposes that two hash tables
be used in a double-buffering manner. A hash table can be in the following three states: A (Active), meaning the table
