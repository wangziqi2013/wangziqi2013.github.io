...
device bandwidth, since log entries are frequently accessed for later reads.
...
before the image itself is accessed. This way, the linear NVM log search can be avoided, which is replaced by a constant 
time operation in the DRAM. The hash table is also periodically checkpointed to the NVM
