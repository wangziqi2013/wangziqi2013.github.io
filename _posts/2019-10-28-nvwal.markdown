...
logically part of the journal. During recovery, the recovery handler scans all blocks in the NVM region, and checks 
their status words. Free blocks and pending blocks are freed, since they are likely not referenced by any valid block.

**To be simple: The allocator is simply there to avoid non-collectable memory leaks. It does not guarantee that all
...
it is acceptable that the post-crash GC overkills as long as the status word is checked every time before use.**