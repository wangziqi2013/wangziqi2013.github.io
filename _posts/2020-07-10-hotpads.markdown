...
3. If sub-objects are allowed, canonical addresses are no longer unique (i.e. a byte can be represented by multiple 
   ...
   each other? Does hardware perform segment intersection?
...
The higher 14 bits of the pointer stores the object sizeOne bit in the pointer is dedicated to whether the pointer is canonical. 
...
the canonical pointer's object has already been cached in the pad by performing a tag comparison. If the comparison indicates a miss, the canonical
address is forwarded to lower level pads, which also perform similar checks until the canonical level is 
reached, in which case the pointer is directly used to address.
