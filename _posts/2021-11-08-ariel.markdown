...
In `handleReadRequest()`, the function first checks whether the operation spans two blocks (multi-block reads that 
...
Fences are handled by setting `isFenced` and `isStalled` to `true`, if the number of pending transactions is greater 
than zero. As discussed earlier, the fence will eventually commit when tha last outstanding memory operation is 
completed.

Flush events are handled by `handleFlushEvent()` similarly to a memory operations, except that it always affects a 
