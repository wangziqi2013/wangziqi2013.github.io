...
to different banks in a certain order to handle the common case of large sequential writes (e.g. logging). By placing the 
bank of the counter update (B / 2) banks away from the data update, we try to minimize the chance that the current counter 
