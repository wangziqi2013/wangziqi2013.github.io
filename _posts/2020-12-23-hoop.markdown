...
4. The paper seems to assume that transactions are committed asynchronously as execution continues after the 
   commit point, since in canonical redo logging, the processor must wait for all log entries to be persisted
   from controllerto NVM before it resumes execution.
   Of course, it is also possible that the paper just assumes that the controller is in the persistent domain,
   in which case controller data is always available after the crash.
...
This paper addresses the above challenges with a combination of techniques. First, 
