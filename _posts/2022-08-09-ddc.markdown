...
With an explicit dictionary compression algorithm, and the dynamically updated LLC side dictionary, the 
paper identifies the main challenge as keeping the dictionary consistent for both in-cache and in-memory blocks. 
