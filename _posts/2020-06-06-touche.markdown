...
(assuming an 8-way set-associative LLC). The 27 bit address the tag stores signatures of the three compressed blocks
in the data slot, or, if compression is not enabled for this slot, stores the 27 bit address tag. 
If one or more compressed blocks are invalid, the signature value is insignificant, and the actual tag comparison logic 
will find this out. The next two bits are valid and dirty bits. In the uncompressed case, their semantics are implied
by their names. If the data slot holds compressed blocks, 
