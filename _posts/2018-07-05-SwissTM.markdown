...
associated with timestamps, which records the commit timestamp of the most recent transaction that wrote to them. 
Transactions keep a read set and write set during read phase execution. The read set maintains addresses of read values
as well as the timestamps when they are being feteched