...
created by concurrent update operations, readers must be able to figure out asd, instead of retrying or blocking on
the temporararily created intermediate state. This is because a blocking read protocol expects some other threads 
to finish the operation and restore the data structure to a 