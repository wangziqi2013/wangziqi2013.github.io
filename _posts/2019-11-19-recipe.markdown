...
or blocking on the temporararily created intermediate state (e.g. blocking on a lock). This is because a blocking read 
protocol expects some other threads to finish the operation and restore the data structure to a within finite number of 
