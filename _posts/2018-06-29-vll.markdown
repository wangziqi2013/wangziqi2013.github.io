...
that are closer to the head, and make sure the current transaction's lock set has empty overlap with their lock sets. 
To simplify complicated set union and intersection, a bloom filter is used to indicate which locks are currently held. 
The SCA thread scans the queue from the head, and summarizes the transaction's lock set into two bloom filters, one for
shared lock and another for exclusive lock. With bloom filters, false positives may happen and a transaction may remain
blocked even if it is eligible for scheduling. This, however, only affects performance, but not correctness.
