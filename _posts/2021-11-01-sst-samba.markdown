...
If `hold` is `true`, which is set by the page table walker when a page walk is going on, the entire hierarchy
...
set to represent the fixed latency of a single memory operation. The link will then be configured as a self link,
i.e., the page table walker just, effectively, calls its own handler function, `recvResp()`, when an event object is 
sent over `to_mem`.
