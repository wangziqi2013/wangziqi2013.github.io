...
and data pages in the simulator can be assigned physical addresses. These addresses are tracked by multiple page table
related structures, namely, data member `PGD`, `PUD`, `PMD`, and `PTE`. These structures are just `std::map`s that 
map virtual addresses on that level (i.e., upper bits of the VA) to physical addresses on the next level (or data page, 
if PTE). New entries are inserted into these maps when the page fault handler returns the allocated physical page, on
a page fault event, and the maps are queried during a hardware page walk to generate the physical addresses of 
...
During page fault handling, normal page walk requests will not be processed (upper level TLBs are not affected), 
which is indicated by the condition checking the `stall` flag at the beginning of `tick()`. If the flag is on, 
then no processing will happen, and the page walker only waits for the fault handling to complete at each tick.

Note that the tracking structures, such as `PENDING_PAGE_FAULTS`, `MAPPED_PAGE_SIZE`, and the page table objects,
are shared across all instances of page walkers, and hence they must always use the same virtual to physical mapping. 
