...
the directory. Conventionally, each bucket is a page allocated from the page buffer pool, but we assume that buckets can
be arbitrarily sized based on the system parameter such as cache line size and the key-value size.