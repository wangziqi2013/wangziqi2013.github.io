...
allow key-value pairs to overflow to adjacent buckets if the current bucket if full, and hash table lookup must also scan 
the next few buckets until the search key is found. 