...
ID of the thread and the thread-local serial number of the transaction instance.
Second, the paper assumes that the system consists of multiple memory controllers, which have non-volatile
...
LAD extends both the L1 cache controller by adding extra states. 
