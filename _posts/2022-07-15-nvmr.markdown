...
3. Idempotent regions (executions whose final state is only a function of the initial register and memory state)
can be delimited as a region that is free of read-write and write-read sequences on the 
