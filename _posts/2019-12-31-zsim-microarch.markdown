...
zSim only models full registers. Partial registers in x86, such as 8-bit and 16-bit registers, will be converted to the 
full-sized counterparts. Modern compilers seldom generate partial register instructions for efficiency reasons, which makes
...
extract its register and memory operands.
