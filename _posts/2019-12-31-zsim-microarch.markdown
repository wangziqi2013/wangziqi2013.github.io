...
be released as soon as possible, since component *Y* will be idle for (C<sub>X</sub> - C<sub>Y</sub> + k) cycles before 
...
To properly model the buffer, we can no longer assume that uops can only be processed in distinct time ranges. Instead,
