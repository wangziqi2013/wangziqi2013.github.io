...
received and released by these components. Only components with buffering capabilities are simulated, such as the issue queue, 
the instruction window, the Register Aliasing Table (RAT) and Register File (RF), the Reorder Buffer (ROB), and the load store 
queue. Non-buffering stages, on the other hand, must always sustain a steady uop throughput, which means that uops always 
...
Y. In this case, the uop is consumed as soon as it is generated by X, which sets the leave cycle of the uop in stage X the
same as the processing cycle, which is C<sub>X</sub>. Second, if C<sub>X</sub> + k > C<sub>Y</sub>, then stage X
must buffer the uop, and only let it leave (C<sub>X</sub> + k - C<sub>Y</sub>) cycles later, since stage Y have not
finished processing the previous uop in program order. 
...
The issue queue consists of an array of timestamps (`buf`) and a tail pointer (`idx`) indicating the producing end of the 
queue. Timestamps in the array are leave cycles of the most recent uops that are stored in that location.