...
zSim simulates all stages for one uop in the order they are generated by the decoder, which is also consistent with program order.
This is even in out-of-order cores, since most components of an out-of-order core are still in-order, such as fetch, decode,
issue, and retirement. Loads and stores are pushed into the load and store queue in program order. In general, this simulation
scheme is faster than tick-by-tick simulation of all components, since in zSim, a component can "skip" idle cycles if the 
previous componeng could not supply, resulting in stalls.
