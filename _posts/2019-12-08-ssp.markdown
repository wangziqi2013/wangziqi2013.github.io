...
hit the cache or are non-transactional).
...
committed, and the memory controller persists the committed word atomically using redo write-ahead logging. The 
memory controller first writes all intended changes to the committed vector to a logging area, flushs the log, and 
then performs the updates in-place. The transaction is fully committed only after the log is committed.
