...
i.e., the number of other accesses between two consecutive accesses to the same page, the paper observes that 
most pages demonstrate larger reuse distances, which implies bad locality.
Second, the paper observes that, by replacing the realistic L1 cache with an ideal L1 that does not incur any access 
