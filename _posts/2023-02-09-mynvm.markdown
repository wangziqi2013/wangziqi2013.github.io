...
the bandwidth problem with big block sizes, the paper proposes to use smaller 4KB blocks for storing the 
key-value pairs. However, reducing block size can affect RocksDB's performance in several ways. First, 
the per-file index stores the first key in each block of the file. When the block size is reduced from 16KB to 4KB,
