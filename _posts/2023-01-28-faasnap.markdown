...
has a more comprehensive view of the working set (e.g., via kernel read ahead). As a result, the snapshot captured 
by FaaSnap suffers fewer page faults because it is more likely to cover more use cases or parameter combinations
than the one generated by REAP. 

After the snapshot is captured, FaaSnap saves the snapshot data to a disk file.
