...
Function `send_command()` (file `connection.py`) generates the request string by calling method `pack()` on 
...
The request string of a command is generated by calling `pack()` on the `class PythonRespSerializer` object.
