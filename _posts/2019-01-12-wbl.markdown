...
WBL works as follows. During normal operation, dirty cache lines can be evicted back to the NVM. These dirty lines belong
to uncommitted transactions, and hence should not be visible to other transactions. On transaction commit, no undo or 
redo log is created. Instead, the 