...
and arguments, and flush the log record, which logically commit the SMO. The thread then locks the affected nodes (or 
use non-blocking help-along protocol), and conduct the update in-place. The log record is removed only after all dirty 
