...
One of the most critical observations is that when the marker check passes, the chance that the block is indeed compressed 
is pretty high, since the probablity of marker conflict exponentially decreases with marker size. In the rare case of marker 
conflicts, one special bit is used to further distinguish between compressed and uncompressed blocks. This special
bit is always checked after marker comparison as the last resort. Since marker conflicts are rare, in most cases 
this special bit 
