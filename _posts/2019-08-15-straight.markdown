...
as allocations in the mapping table should also be rolled back. With the first design, the paper suggests that a reorder
buffer walker should be invoked and undo all changes made by the speculation path (the ROB entry should record the 
before-image of the entry it affects for such undo operations). 