...
a larger, but still aligned granularity. For example, DMC compresses with LZ77 in 1KB blocks, while with LCP in 32KB
continuous ranges, which is larger than the original LCP proposal (4KB).
