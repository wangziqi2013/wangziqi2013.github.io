...
We have the following ordering: (1) T2 unlock A < T1 access A; (2) T1 access B < T2 lock B. If T1 accesses A before T1 accesses
B, then according to transitivity we have T2 unlock A < T2 lock B, which violates the 2PL rule. 
