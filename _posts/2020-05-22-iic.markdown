...
1. This paper uses a different definition of full associativity. It mostly means that an address can be stored anywhere 
   on the entire address space in other occasions, and even earlier part
...
Note that all tags within a bucket share one "chain" field, which saves some storage as 
