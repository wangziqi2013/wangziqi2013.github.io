...
system, but also claims that ARIES does not preclude other design options. In either options, transactions must acquire 
...
log records is consistent with the order of actual page modifications happening in real time. If this property is violated,
it is possible that two log records on the same page are written out-of-order. During redo phase, these two entries will
then be applied also in the wrong order, causing data corruption in some cases (e.g. redo logging is operational). 