...
(GC), which is also unfortunately the most likely bottleneck in almost all implementations. 
...
dead (i.e., not being referenced by the index). Dead entries in log-structured key-value stored can be introduced 
