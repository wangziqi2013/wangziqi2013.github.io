...
bits per tag, an offset field, metadata field and compression type field are also added for placement and decompression 
...
same time.
The two-bit algorithm selection field
