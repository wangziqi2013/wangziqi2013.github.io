...
message is sent to servers that the transaction has accessed. The message consists of the transaction's write set, and all 
UW(x) sets collected during the read phase. We give a detailed description below how data server handles the pre-write message. 
First, for all items in the write set, the server fetches the corresponding rts and wts. 
