...
Another important data structure is the transaction table. Every data server has a transaction table which records the 
...
the validation stage, which is a must in some early interval-based distributed protocols.
