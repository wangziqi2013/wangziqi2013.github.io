...
Under distributed settings, the database is divided into multiple partitions. Each partition is hosted by a server. 
User requests are sent to one of the front-end servers, which are responsible for issuing data requests to the backend
and performing concurrency control logic. 