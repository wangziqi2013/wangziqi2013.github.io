...
when a conflicting lock mode is acquired, soft locks never block. Instead, a list of current lock owners are returned, which 
serves as the basis for conflict resolution during validation.
