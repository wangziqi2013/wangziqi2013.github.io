...
no context switch on the core happens, we compare `curCycle` after adjustment with the end cycle of the next interval,`core->phaseEndCycle`. If `curCycle` is even larger than the end cycle of the next interval, meaning that the core has 
already been running "to far ahead" into the future compared with the global zll clock, then we slow it down a little
by skipping the bound phase. 
