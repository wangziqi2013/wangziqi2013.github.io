...
Event recorders are implemented in file event\_recorder.h as `class EventRecorder`. For single-domain contention simulation,
it is just a stack of timing records, `trStack`, plus a memory allocator, `slabAlloc`, as we have seen above. The event
recorder provides interfaces for pushing and poping a timing record, and also for accessing timing records in-place. 
