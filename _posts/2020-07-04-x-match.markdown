...
a partial match entry with the least number of byte literals, and then partial match entries with the smallest index to
minimize the codeword size (**Note: I can't see how smallest index helps in this case, but it is just a minor glitch**). 
