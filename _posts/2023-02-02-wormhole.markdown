...
However, since the radix tree only stores prefixes (possibly with the end mark) of the actual keys stored in the 
leaf nodes, during the radix tree traversal, there may be no next node in the current level given
