...
In all cases, if the address contains a valid block previously, and the block uses shared base or diff, the reference
counts of these bases and diffs should be decremented. If a reference count drops to zero, the block or diff will be
freed.