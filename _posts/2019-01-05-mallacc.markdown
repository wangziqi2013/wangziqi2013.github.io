...
The current design is described as follows. After obtaining the size class index, malloc removes the first element
from the free list using a "pop" instruction. The "pop" instruction writes the current "head" in a register, and 
shifts the "next" field into the "head" field. The "next" field is then cleared. If the "pop" instruction