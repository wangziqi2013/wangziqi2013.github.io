...
Note that only connected ports in the Python configuration file are being initialized as link objects. 
Components and subcomponents can retrieve the initialized link objects using the port name in the 
configuration file, by calling the member function `configureLink()` (which has several different flavors), 
and further configure them.
In other words, in SST, link objects are first initialized before their components are, and the initialization
will assign link objects with the port name used in the configuration file At component initialization time,
link objects are retrieved given the component ID and the port name, and is eventually bounded with a call back
...
After `prepareLinks()`, connected link objects are created and hold a reference to each other, but configuration
