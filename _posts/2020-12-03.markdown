...
that only one of them is zero. Each entry is associated with a counter that tracks the number of times the 
value in this entry occurs since last time it was inserted or promoted.
