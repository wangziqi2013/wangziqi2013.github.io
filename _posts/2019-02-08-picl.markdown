...
back immediately. In fact, the paper proposes an optimization in which write operations resulting from the write back
are combined into a bulk write of the size of the row buffer in NVM. This is to 