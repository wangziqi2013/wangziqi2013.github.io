...
Path compression, however, is not performed during deletes. In the above example, if the second key inserted into the 
...
checking, since two HAMTs having the same content may not have identical tree struct. In other words, the shape of the
