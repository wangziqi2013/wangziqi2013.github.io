...
Based on the physical location, NOVA metadata is divided into two types: An in-memory part which supports efficient 
...
user applications issue file accesses, the in-memory representation of the inode, directory, etc. are initialized
by scanning the log and applying all changes.