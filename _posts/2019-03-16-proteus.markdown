...
After the slot becomes ready, it can be selected by the write back hardware and then flushed. Entries in the queue can be 
flushed out-of-order to maximize bandwidth, which also enables some optimizations.