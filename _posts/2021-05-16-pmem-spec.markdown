...
1. What if the data race free program is implemented not with critical sections, but with 2PL? Which version number
   should we use? I understand not being able to support more complicated transactional or FASE model and only
   supporting lock-based ones (many existing mechanism makes similar assumptions). But at least 2PL should be 
   supported?
...
acquisition.
To this end, the paper proposes that the compiler maintains a version number for each critical section lock, which is 
incremented every time it is acquired
