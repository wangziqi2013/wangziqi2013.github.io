...
acquisition.
To this end, the paper proposes that the compiler maintains a per-process global version number for all 
critical section locks within the application, which is incremented every time a critical section is entered.
...
The NVM controller always remembers the version of the last write 
