...
block where smaller blocks can be allocated from using special allocator interface. To aid crash recovery, each allocation
...
In addition, blocks are assumed to consist of objects that are linked using pointers. 